...
namespace WebApplication1
{
    ...
    public partial class TopluTasimaS : System.Web.UI.Page
    {
        ...
        protected void btnSorgula_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lblSonuc.Text = "⚠️ Lütfen başlangıç ve hedef durak seçiniz.";
                    return;
                }

                if (baslangic == hedef)
                {
                    lblSonuc.Text = "⚠️ Başlangıç ve hedef durak aynı olamaz.";
                    return;
                }
                ...
                if (tekHat != null)
                {
                    ...
                    lblSonuc.Text = $"🚍 <b>{baslangic}</b> durağından <span style='color:{tekHat.HatAdi.ToLower()}'><b>{tekHat.HatAdi}</b></span> otobüsüne binin ve 🎯 <b>{hedef}</b> durağında inin.<br/>" +
                                    $"🛣️ Mesafe: <b>{toplamMesafe} km</b><br/>" +
                                    $"⏱️ Tahmini süre: <b>{sure} dakika</b>";
                }
                else
                {
        ...
        private void RotayiHesaplaVeGoster(string baslangic, string hedef, List<string> yol)
        {
            try
            {
                ...
                if (hatlar.Count > 1)
                {
                    mesaj = "⚠️ Bu iki durak arasında direkt bir otobüs hattı yok. Aktarma yapmanız gerekiyor.<br/><br/>";
                }
                ...
                mesaj += $"🛣️ Toplam mesafe: <b>{toplamMesafeAktarma:F1} km</b><br/>";
                mesaj += $"⏱️ Tahmini süre: <b>{(int)Math.Round(toplamSure)} dakika</b> ({aktarmaSayisi} aktarma)";
